...
enum PiPa {
    pipa_vertex_shader,
    pipa_fragment_shader,
    pipa_tess_control_shader,
    pipa_tess_eval_shader,
    pipa_vertex_stride,
    pipa_topology,
    pipa_patch_control_points,
    pipa_polygon_mode,
    pipa_cull_mode,
    pipa_num
};

static bool create_graphics_pipelines(VkPipeline*    pipelines,
                                      uint32_t       num_pipelines,
                                      const uint8_t* pipeline_data,
                                      uint8_t**      shaders,
                                      uint32_t       num_shaders,
                                      const VkVertexInputAttributeDescription* vertex_attributes,
                                      uint32_t       num_vertex_attributes)
{
    VkShaderModule shader_ids[32];

    assert(num_shaders <= mstd::array_size(shader_ids));

    for (uint32_t id = 0; id < num_shaders; id++)
        shader_ids[id] = load_shader(shaders[id]);

    for (uint32_t i_pipeline = 0; i_pipeline < num_pipelines; i_pipeline++, pipeline_data++) {

        const uint8_t* cur_data = pipeline_data;

        assert(pipelines[i_pipeline] == VK_NULL_HANDLE);
        ...
        for (uint32_t i_shader = 0; i_shader < mstd::array_size(shader_stages); i_shader++) {
            const uint32_t id = *cur_data;
            cur_data += num_pipelines;

            assert(id < num_shaders);
            shader_stages[i_shader].module = shader_ids[id];
        }
        ...
        const uint32_t vertex_stride = *cur_data;
        cur_data += num_pipelines;
        assert(vertex_stride > 0 && vertex_stride <= 128);
        vertex_bindings[0].stride = vertex_stride;
        ...
        const uint32_t topology = *cur_data;
        cur_data += num_pipelines;
        assert(topology <= VK_PRIMITIVE_TOPOLOGY_PATCH_LIST);
        input_assembly_state.topology = VkPrimitiveTopology(topology);
        ...
        const uint32_t patch_control_points = *cur_data;
        cur_data += num_pipelines;
        assert(patch_control_points <= 16);
        tessellation_state.patchControlPoints = patch_control_points;
        const uint32_t num_stages = patch_control_points ? 4 : 2;
        ...
        const uint32_t polygon_mode = *cur_data;
        cur_data += num_pipelines;
        assert(polygon_mode <= VK_POLYGON_MODE_POINT);
        rasterization_state.polygonMode = VkPolygonMode(polygon_mode);

        const uint32_t cull_mode = *cur_data;
        cur_data += num_pipelines;
        assert(cull_mode <= VK_CULL_MODE_FRONT_AND_BACK);
        rasterization_state.cullMode = static_cast<VkCullModeFlagBits>(cull_mode);
        ...
        const VkResult res = CHK(vkCreateGraphicsPipelines(vk_dev,
                                                           ...
                                                           nullptr,
                                                           &pipelines[i_pipeline]));
        if (res != VK_SUCCESS)
            return false;

        assert(pipeline_data + num_pipelines * pipa_num == cur_data);
    }
...
{
    static uint8_t* shaders[] = {
        shader_simple_vert,
        shader_phong_frag
    };

    static const uint8_t pipeline_params[] = {
        0, // vertex shader
        0,
        1, // fragment shader
        1,
        0, // tess control shader
        0,
        0, // tess eval shader
        0,
        static_cast<uint8_t>(sizeof(Vertex)),
        static_cast<uint8_t>(sizeof(Vertex)),
        VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
        VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
        0, // patch control points
        0,
        VK_POLYGON_MODE_FILL,
    ...
    return create_graphics_pipelines(vk_gr_pipeline,
                                     mstd::array_size(vk_gr_pipeline),
                                     pipeline_params,
                                     shaders,
                                     mstd::array_size(shaders),
                                     vertex_attributes,
                                     mstd::array_size(vertex_attributes));
}

static bool create_quadratic_patch_graphics_pipeline()
{
    static uint8_t* shaders[] = {
        shader_rounded_cube_vert,
        shader_phong_frag,
        shader_bezier_surface_quadratic_tesc,
        shader_bezier_surface_quadratic_tese
    };
...
static bool create_pipelines()
{
    if (what_geometry == geom_cube)
        ...
        return create_quadratic_patch_graphics_pipeline();
    else
